<!-- compressed 2026-05-25 -->

# CLAUDE.md — Orchid Framework (v2.5)
...
`audit.py` — `AuditStore` (JSONL, daily rotation), `AuditAction` constants (incl. `SCHEDULED_TASK_RUN`, `SCHEDULED_TASK_FAILED`), `make_event()`.
...
`types.py` — `ScheduledTask` (task_id `stask_*`, owner_id, name, schedule, task_type, config, enabled, notify_on_*), `TaskRun` (run_id `run_*`, status `running|success|failure|timeout`).
`store.py` — `TaskRunStore`: append-only JSONL `~/.config/orchid/cron/runs.jsonl`; 30-day prune on init; thread-safe; `append(run)`, `get_runs(task_id, owner_id, limit)` newest-first.
`executor.py` — `TaskExecutor.execute(task_dict, owner_id) -> TaskRun`: dispatches by `task_type`; `agent_prompt` uses provider registry; `mcp_tool` uses MCPManager; `shell` uses `bash()`; never raises.
`engine.py` — `CronEngine`: APScheduler `BackgroundScheduler` (UTC); `get_engine()` double-checked singleton; `reset_engine()` for tests; `start()` loads all enabled tasks from store; `add_or_update_task()`, `remove_task()`, `run_now()` (daemon thread).
`api.py` — `register_routes(app)`: local imports only (no `from __future__ import annotations`); endpoints: `GET/POST /api/scheduler/tasks`, `GET/PUT/DELETE /api/scheduler/tasks/{id}`, `POST /api/scheduler/tasks/{id}/run`, `GET /api/scheduler/tasks/{id}/runs`, `GET /api/scheduler/runs`.
...
## Current State
**V2.5 Complete. 1550+ tests passing (134 auth + 68 Phase 1–6 reliability + 56 cron tests).**
...
*   **T285–T297** Cron scheduled task manager: types, JSONL run store, executor, APScheduler engine, REST API, web server wiring, 56 tests (D0061).
